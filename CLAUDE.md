...
Entry point is [src/main.rs](src/main.rs), using `clap` derive macros. Global options (`-r/--repository`, defaulting to `~/.dotty` or `$DOTTY_REPOSITORY`; `-R/--root`, the directory dotfiles live relative to) are parsed once in `Opts`, resolved to canonical paths, then dispatched to a subcommand handler in [src/cmds.rs](src/cmds.rs). Each subcommand (`init`, `clone`, `add`, `restore`, `commit`, `sync`, `update`, `status`, `remove`) is a thin free function in `cmds.rs` that composes helpers from `src/utils/`.
...
- [src/utils/git.rs](src/utils/git.rs) — all `git2` usage: init/open/clone (with recursive submodule init), staging/committing, submodule add/update, working-tree status (`pending_changes`), and a hand-rolled fetch+merge+push flow for `sync` (fast-forward when possible, otherwise a real merge commit; conflicts abort with an error rather than being resolved automatically).
...
- `add`, `restore`, `commit` and `remove` take `--dry-run`, which must genuinely change nothing — each has an end-to-end test asserting the machine is untouched afterwards. The planning path mirrors the real one rather than sharing it, so a change to one needs the same change in the other.
- Files displaced by `restore --overwrite` go to `<root>/.dotty-backup-<epoch>`, beside the dotfiles rather than under `$TMPDIR`: they are the user's real configuration, and the system temp directory is cleared on a schedule nobody controls.
- `sync` commits before it fetches, because a merge checks out over the working tree. What counts as a change is decided by `git::pending_changes`, and its two exclusions are load-bearing rather than incidental: **untracked files** are reported and skipped (dotty cannot tell a hand-copied config from a `.DS_Store`), and **dirt inside a submodule** is ignored entirely — a submodule's files belong to its own repository, so counting them left sync refusing to run over a file dotty had no way to commit. A moved submodule *pointer* is still a change.
- Anything in `sync` that moves `HEAD` must also check the submodules out (`checkout_submodules`). A merge only updates the recorded commit; leaving the submodule's own working tree behind reports the repository as dirty, so a sync that appeared to succeed broke the next one.
- `normal_merge` goes through `Repository::merge` rather than `merge_trees` specifically because that writes `MERGE_HEAD`. Conflict markers land in files symlinked into `$HOME`, so `git merge --abort` has to work; without `MERGE_HEAD` neither `--abort` nor `--continue` did.
