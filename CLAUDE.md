...
- `add`, `restore` and `remove` take `--dry-run`, which must genuinely change nothing — each has an end-to-end test asserting the machine is untouched afterwards. The planning path mirrors the real one rather than sharing it, so a change to one needs the same change in the other.
