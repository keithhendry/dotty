...
- Tagging is **manual**: run the `tag.yaml` workflow by hand when the commits on `main` add up to a release. It derives the next semantic version from the conventional commits since the last tag.
